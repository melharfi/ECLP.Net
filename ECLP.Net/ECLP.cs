...
namespace ECLP.Net
{
    public class ECLP
    {
        ...
        /// <summary>
        /// Empty constructor, used when parsing a file using ParseFile methode
        /// </summary>
        public ECLP()
        {
        ...
        /// <summary>
        /// Parse file command and return list of CommandResult
        /// </summary>
        /// <param name="path">File path that containe commands</param>
        /// <param name="usingRegex">If true the parsing function will use regex patterns, if not it will use side by side words check algorithm, the way choise is given to user is because Regex sometime not used in some assemblies like wrapped or mono modified assemblies and if regex fail to work perfectly you still can use the side by side one</param>
        /// <returns>list of parsed command as List<CommandResult></returns>
        public List<CommandResult> ParseFile(string path, bool usingRegex = true)
        {
            ...
            foreach (string line in lines)
            {
                ...
                if (usingRegex)
                {
                    commandResults.Add(UsingRegex());
                }
                else
                {
                    commandResults.Add(WithoutRegex());
                }
            }
